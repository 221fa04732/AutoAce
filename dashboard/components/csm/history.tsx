...
import {HistoryIcon, CarFront, CheckCircle, AlertTriangle, Clock } from "lucide-react"
...
export default function History() {
  ...
  const fetchData = async () => {
    ...
    setError(false)
    try {
      const res = await axios.get(`${BASE_URL}/api/cms/history`)
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      return dealerMatch && statusMatch && fromMatch && toMatch
    })
  }, [data, dealerFilter, statusFilter, fromDate, toDate])
  ...
  return (
    <div className="relative bg-zinc-950 rounded-xl border border-neutral-700 px-5 py-4 shadow-md hover:bg-zinc-900 hover:border-neutral-500 transition hover:-translate-y-1">
      ...
      <div className="mt-2 text-xs text-slate-500 text-right">
        Reviewed: {new Date(history.created_at).toLocaleString()}
      </div>
    </div>
  )
}
